...
from .groq_api_utility import groq_api_call
...
def get_top_k_docs(query, class_id):
    ...
    relevant_docs = vectordb.max_marginal_relevance_search(query,
                                                           k=fetch_k,
                                                           filter={
                                                               "source": f"{BASE_TRANSCRIPT_PATH}{class_id}_transcript.txt"})
...
def get_contextualized_question(chat_history, query):
    if chat_history:
        ...
        <instructions>
            ...
            1. Do not add anything extra from your side
            2. Formulate a standalone question which can be understood without the chat history only if latest user question has pronouns or articles referring to someone or something in the chat-history, otherwise return it as is.
            3. You MUST NOT answer the question
            4. Return only the question in the given ideal format:
                question: <formulated question>. Don't add any explanation or anything other then given ideal format.
        </instructions>
        """

        return groq_api_call(prompt=contextualize_q_system_prompt, query=query)
    else:
        return query


def question_answer(class_id, member_id, query):
    print("inside the groq api ...............................................")
    ...
    """
    return groq_api_call(prompt=qa_system_prompt, query=contextualized_question, temperature=0.3)
